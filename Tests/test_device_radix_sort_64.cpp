...
void generate_random_data_drs(const Resource &device,
                              std::vector<uint64_t> &data, uint64_t seed,
                              float entropy = 1.0f) {
  Random<Resource> rng(device, 12);
  rng.init(seed, 0);
  DeviceBuffer<uint64_t> d_data(data.size(), device.id());
  uint64_t min_val = 0;
  uint64_t max_val;
  if (entropy >= 0.99f) {
    max_val = UINT64_MAX;
...
TEST_CASE("DeviceRadixSort Key-Value Pairs - Small",
  ...
  SECTION("Sort 1K elements") {
    std::vector<uint64_t> h_keys(size);
    std::vector<uint64_t> h_payloads(size);
    generate_random_data_drs(device, h_keys, 12345);
    std::iota(h_payloads.begin(), h_payloads.end(), 0);

    DeviceBuffer<uint64_t> d_keys(size, device.id());
    DeviceBuffer<uint64_t> d_payloads(size, device.id());
    d_keys.copy_from_host(h_keys.data(), size);
    d_payloads.copy_from_host(h_payloads.data(), size);

    DeviceBuffer<uint64_t> d_alt_keys(size, device.id());
    DeviceBuffer<uint64_t> d_alt_payloads(size, device.id());
    DeviceBuffer<uint64_t> d_globalHistogram(DRS_RADIX * 8, device.id());
    const uint64_t threadBlocks = (size + DRS_PART_SIZE - 1) / DRS_PART_SIZE;
    DeviceBuffer<uint64_t> d_passHistogram(DRS_RADIX * threadBlocks,
                                           device.id());
    ...
    std::vector<uint64_t> h_sorted_keys(size);
    std::vector<uint64_t> h_sorted_payloads(size);
    ...
    std::vector<std::pair<uint64_t, uint64_t>> original(size);
